...
is_empty([$id, $request_id], '/ster-opdrachten', 'Deze link is niet geldig');
...
if ($steropdracht['status'] >= 3) {
    log_action('steropdracht.edit_denied_because_not_done');
...
END;
        ...
        log_action('steropdracht.join_accepted');

        redirect('/ster-opdrachten/view/' . $id, 'Verzoek geaccepteerd');
    } else {
        log_action('steropdracht.join_denied');
